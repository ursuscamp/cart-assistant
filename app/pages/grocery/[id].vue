  ...
  <div v-if="groceryStore.currentList" class="max-w-5xl mx-auto space-y-4 lg:space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div class="flex items-center gap-3">
        <NuxtLink
          to="/grocery"
          class="p-2 rounded-lg text-bark-400 hover:text-white hover:bg-bark-800 transition-colors touch-manipulation"
        >
      ...
      <div class="flex gap-2">
        <button
          ...
          class="btn-secondary flex-1 sm:flex-none touch-manipulation"
        >
    ...
    <div class="card">
      <div class="card-body">
        <div class="flex items-center justify-between mb-3">
          ...
          <span class="text-sm font-semibold text-forest-400"
    ...
    <div class="space-y-4">
      <div
        v-for="section in localSections.filter((s) => s.items.length > 0)"
        :key="section.id"
            ...
            <div class="flex items-center gap-3">
              <button
                @click="
                  ...
                  if (!section.isClosed) manuallyClosedSections.value.delete(section.id);
                  else manuallyClosedSections.value.add(section.id);
                "
                class="w-8 h-8 rounded-lg bg-forest-600/20 flex items-center justify-center flex-shrink-0 touch-manipulation"
              >
                <svg
                  :class="[
                    'w-4 h-4 text-forest-400 transition-transform',
                    section.isClosed ? '' : 'rotate-90'
                  ]"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
              ...
              <span class="badge">{{
                section.items.filter((i) => !i.is_checked).length || '✓'
              }}</span>
            </div>
          </div>
        </div>
        <div v-if="!section.isClosed">
          ...
          >
            <template #item="{ element: item }">
              <div
                :class="[
                  'flex items-center gap-2 lg:gap-4 px-3 lg:px-4 py-3 lg:py-4 transition-all duration-200 touch-manipulation',
                  item.is_checked ? 'bg-bark-900/30' : 'hover:bg-bark-800/30'
                ]"
              >
                <div class="flex-shrink-0 drag-handle cursor-grab">
                  <svg
                    class="w-4 h-4 text-bark-500"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <span
                  :class="[
                    'flex-1 min-w-0 text-base lg:text-lg',
                ...
                <select
                  ...
                  class="bg-bark-800 border border-bark-700 text-white text-xs lg:text-sm rounded-lg px-2 py-1.5 lg:px-3 lg:py-2 focus:outline-none focus:ring-1 focus:ring-forest-500 w-24 lg:w-32 flex-shrink-0"
                >
                  <option v-for="s in allSections" :key="s.id" :value="s.id">{{ s.name }}</option>
                </select>
                <button
                  @click="removeItem(item)"
                  class="p-2 rounded-lg text-bark-500 hover:text-red-400 hover:bg-red-900/20 transition-colors flex-shrink-0 touch-manipulation"
                >
                  <svg
...
const localSections = ref<
  Array<{ id: number; name: string; items: GroceryItem[]; isClosed?: boolean }>
>([]);
const manuallyClosedSections = ref(new Set<number>());

const totalCount = computed(
  () => groceryStore.currentList?.sections?.reduce((sum, s) => sum + s.items.length, 0) || 0
);
const checkedCount = computed(
  () =>
    groceryStore.currentList?.sections?.reduce(
      (sum, s) => sum + s.items.filter((i) => i.is_checked).length,
      0
    ) || 0
);
...
async function onDragEnd(section: { id: number; items: GroceryItem[] }) {
  ...
  for (const item of section.items) {
    if (item.section_id !== section.id) {
      await groceryStore.updateItem(groceryStore.currentList.id, item.id, {
...
async function toggleItem(item: GroceryItem) {
  ...
  const originalItemIdx = section?.items.findIndex((i) => i.id === item.id) ?? -1;

  if (section && originalItemIdx >= 0) {
    const [movedItem] = section.items.splice(originalItemIdx, 1);

    if (item.is_checked) {
      section.items.push(movedItem);
    } else {
      section.items.unshift(movedItem);
      manuallyClosedSections.value.delete(section.id);
    }
  }

  try {
    await groceryStore.toggleChecked(
      groceryStore.currentList.id,
      item.id,
      item.is_checked,
      groceryStore.currentList
    );
    ...
    if (section) {
      const currentIdx = section.items.findIndex((i) => i.id === item.id);
      if (currentIdx >= 0) section.items.splice(currentIdx, 1);
      if (originalItemIdx >= 0) section.items.splice(originalItemIdx, 0, item);
    }
