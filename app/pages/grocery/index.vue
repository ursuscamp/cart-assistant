      ...
      >
        <NuxtLink :to="`/grocery/${list.id}`" class="block">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-6 gap-4">
            <div class="flex items-center gap-4 min-w-0">
              <div
                class="w-12 h-12 rounded-xl bg-forest-600/20 flex items-center justify-center flex-shrink-0"
              >
                <svg
                  class="w-6 h-6 text-forest-400"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div class="min-w-0">
                <h3
                  class="text-lg font-semibold text-white group-hover:text-forest-400 transition-colors truncate"
                >
                  {{ list.name }}
                </h3>
                <div class="flex flex-wrap items-center gap-2 mt-1">
                  ...
                  <span v-if="list.checked_count" class="text-sm text-forest-400"
                    >{{ list.checked_count }} checked</span
                  >
                </div>
              </div>
            </div>
        ...
        <div v-if="list.sources?.length" class="px-4 sm:px-6 pb-4">
          <div class="flex flex-wrap gap-2">
            ...
            <span v-if="list.sources.length > 5" class="chip"
              >+{{ list.sources.length - 5 }} more</span
            >
          </div>
        </div>
        <div class="px-4 sm:px-6 pb-4 flex justify-end">
          <button
            ...
            class="btn-ghost text-bark-400 hover:text-red-400 text-sm touch-manipulation"
          >
