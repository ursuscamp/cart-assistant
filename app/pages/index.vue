			...
			<div class="card">
				<div class="card-header bg-gradient-to-r from-forest-700 to-forest-800">
					<div class="flex items-center gap-3">
						...
						<div>
							<h2 class="text-lg font-semibold text-white">Quick Create
								Grocery List</h2>
				...
				<div class="card-body space-y-4">
					<p class="text-sm text-bark-300">Select lists to combine into your grocery list:
					</p>

					<div v-if="listsStore.lists.length > 0"
						class="space-y-2 max-h-80 overflow-y-auto scrollbar-thin">
						<label v-for="list in sortedLists" :key="list.id" :class="[
							...
							<div class="flex items-center gap-3">
								...
								<span class="font-medium text-white">{{ list.name
								}}</span>
							</div>
					...
					<div v-else class="text-center py-8">
						<div
							class="w-16 h-16 mx-auto rounded-full bg-bark-800 flex items-center justify-center mb-4">
							<svg class="w-8 h-8 text-bark-500" fill="none"
								stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round"
									stroke-width="2"
					...
					<div>
						...
						<input v-model="listName" type="text" class="input-field"
							placeholder="Week of Feb 1" />
					</div>

					<button @click="createGroceryList"
			...
			<div class="space-y-6">
				...
				<div class="card">
					<div class="section-header">
						<div class="flex items-center gap-3">
							<div
								...
								<svg class="w-5 h-5 text-secondary" fill="none"
									stroke="currentColor" viewBox="0 0 24 24">
					...
					<div class="card-body">
						<div v-if="groceryStore.lists.length > 0" class="space-y-2">
							<NuxtLink v-for="list in groceryStore.lists.slice(0, 5)"
								:key="list.id" :to="`/grocery/${list.id}`"
								class="list-item group">
								<div
									class="w-10 h-10 rounded-lg bg-forest-600/20 flex items-center justify-center">
									<svg class="w-5 h-5 text-forest-400" fill="none"
										stroke="currentColor"
										viewBox="0 0 24 24">
										<path stroke-linecap="round"
								...
								<div class="flex-1 min-w-0">
									<p
										class="font-medium text-white group-hover:text-forest-400 transition-colors">
										{{ list.name }}</p>
									<p class="text-sm text-bark-400">
										{{ list.item_count || 0 }} items
										<span v-if="list.checked_count"
											class="text-forest-400 ml-2">
											{{ list.checked_count }} checked
										</span>
									</p>
								</div>
								<svg class="w-5 h-5 text-bark-500 group-hover:text-forest-400 transition-colors"
				...
				<div class="card">
					<div class="section-header">
						<div class="flex items-center gap-3">
							<div
								class="w-8 h-8 rounded-lg bg-moss-500/20 flex items-center justify-center">
								<svg class="w-5 h-5 text-moss-400" fill="none"
									stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round"
										stroke-linejoin="round" stroke-width="2"
