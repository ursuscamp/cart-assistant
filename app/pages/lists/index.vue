<template>
  <div class="max-w-6xl mx-auto space-y-6">
    ...
    <div v-if="listsStore.lists.length > 0" class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <div class="card-header bg-gradient-to-r from-forest-700/50 to-forest-800/30">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="font-semibold text-white">{{ list.name }}</h3>
            ...
            <div class="flex gap-1">
              <button
                ...
                class="p-2 rounded-lg text-bark-400 hover:text-white hover:bg-white/10 transition-colors"
              >
              ...
              <button
                ...
                class="p-2 rounded-lg text-bark-400 hover:text-red-400 hover:bg-red-900/20 transition-colors"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="list.is_regular_items" class="px-4 pb-3">
          <span class="badge bg-forest-600">Regular Items</span>
        </div>
      </div>
    </div>
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <label for="regular-items" class="text-sm text-bark-300 cursor-pointer">
            Regular items list (appears at the top when creating grocery lists)
          </label>
        </div>
        ...
        <div>
          <div class="flex items-center justify-between mb-3">
            ...
            <button @click="addIngredient" class="text-sm text-forest-400 hover:text-forest-300">
              + Add Ingredient
            </button>
          </div>
          <div class="space-y-3 max-h-60 overflow-y-auto scrollbar-thin">
            <div v-for="(ing, index) in listForm.ingredients" :key="index" class="flex gap-3">
              <input
                v-model="ing.name"
                type="text"
              ...
              <button
                ...
                class="p-3 text-bark-400 hover:text-red-400 transition-colors"
              >
    ...
    <Modal :show="showImportModal" title="Import Lists from Text" @close="closeImportModal" size="lg">
      <div class="space-y-4">
        <p class="text-sm text-bark-300">Paste your recipe or shopping lists below. We'll parse them automatically.</p>
        <textarea
          v-model="importText"
          class="input-field min-h-48 font-mono text-sm"
          ...
          <div v-for="(list, index) in importPreview" :key="index" class="card">
            <div class="p-4">
              ...
              <div class="mt-3 ml-8 flex flex-wrap gap-1.5">
                <span
                  v-for="ing in list.ingredients.slice(0, 8)"
                  :key="ing.name"
                  class="chip"
                >
                  {{ ing.name }} {{ ing.quantity ? `(${ing.quantity})` : '' }}
                </span>
                <span v-if="list.ingredients.length > 8" class="chip">
                  +{{ list.ingredients.length - 8 }} more
                </span>
              </div>
            </div>
          </div>
    ...
    <Modal :show="showDeleteModal" title="Delete List" @close="showDeleteModal = false">
      <p class="text-bark-300">
        Are you sure you want to delete "{{ deletingList?.name }}"? This action cannot be undone.
      </p>
      <template #footer>
        <button @click="showDeleteModal = false" class="btn-secondary">Cancel</button>
        <button @click="deleteList" class="bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-lg font-medium transition-colors">
          Delete
        </button>
      </template>
    </Modal>
