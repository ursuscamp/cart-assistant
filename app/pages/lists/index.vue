  ...
  <div class="max-w-6xl mx-auto space-y-6">
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-1 flex-shrink-0">
              <button
                ...
                class="p-2 rounded-lg text-bark-400 hover:text-white hover:bg-white/10 transition-colors touch-manipulation"
              >
              ...
              <button
                ...
                class="p-2 rounded-lg text-bark-400 hover:text-red-400 hover:bg-red-900/20 transition-colors touch-manipulation"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    stroke-linecap="round"
          ...
          <NuxtLink :to="`/lists/${list.id}`" class="block touch-manipulation">
            <div class="flex flex-wrap gap-1.5">
              <span
                ...
                :key="ing.id"
                class="chip-clickable text-xs"
                >{{ ing.name }}</span
              >
              <span v-if="(list.ingredient_count || 0) > 5" class="chip text-xs"
                >+{{ (list.ingredient_count || 0) - 5 }} more</span
              >
            </div>
          </NuxtLink>
        </div>
    ...
    <div v-else class="card">
      <div class="card-body text-center py-16">
        <div
          class="w-20 h-20 mx-auto rounded-full bg-bark-800 flex items-center justify-center mb-6"
        >
          <svg
            class="w-10 h-10 text-bark-500"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
    ...
    >
      <div class="space-y-4">
        ...
        <div class="flex items-center gap-3">
          ...
          />
          <label for="regular-items" class="text-sm text-bark-300 cursor-pointer"
            >Regular items list</label
          >
        </div>
        ...
        <div>
          <div class="flex items-center justify-between mb-3">
            ...
            <button
              @click="addIngredient"
              class="text-sm text-forest-400 hover:text-forest-300 touch-manipulation"
            >
              + Add
            </button>
          </div>
          <div class="space-y-3 max-h-60 overflow-y-auto scrollbar-thin">
            <div v-for="(ing, index) in listForm.ingredients" :key="index" class="flex gap-2">
              ...
              <button
                ...
                class="p-2 text-bark-400 hover:text-red-400 transition-colors touch-manipulation"
              >
    ...
    >
      <div class="space-y-4 max-h-[calc(100vh-220px)] overflow-y-auto pr-2 scrollbar-thin">
        ...
        <textarea
          v-model="importText"
          class="input-field min-h-32 font-mono text-sm"
        ...
        <div v-if="importPreview.length > 0" class="space-y-3">
          ...
          <h4 class="font-medium text-white">
            Preview ({{ importPreview.filter((l) => l.selected).length }} selected)
          </h4>
            ...
            <div v-for="(list, index) in importPreview" :key="index" class="card">
              <div class="p-3">
                ...
                <div class="mt-2 ml-8 flex flex-wrap gap-1">
                  <span
                    v-for="ing in list.ingredients.slice(0, 6)"
                    :key="ing.name"
                    class="chip text-xs"
                    >{{ ing.name }} {{ ing.quantity ? `(${ing.quantity})` : '' }}</span
                  >
                  <span v-if="list.ingredients.length > 6" class="chip text-xs"
                    >+{{ list.ingredients.length - 6 }} more</span
                  >
                </div>
              </div>
            </div>
    ...
    <Modal :show="showDeleteModal" title="Delete List" @close="showDeleteModal = false">
      ...
      <template #footer>
        ...
        <button
          @click="deleteList"
          class="bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-lg font-medium transition-colors touch-manipulation"
        >
          Delete
        </button>
      </template>
...
function openEditModal(list: List) {
  ...
  listForm.ingredients = listIngredientsCache.value.get(list.id)?.map((i) => ({ ...i })) || [
    { name: '', quantity: '' }
  ];
  showListModal.value = true;
}
