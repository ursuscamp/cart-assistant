      ...
      <div class="card flex flex-col">
        ...
        <div class="p-3 lg:p-4 space-y-2 overflow-y-auto scrollbar-thin max-h-[calc(100vh-320px)]">
          <draggable
            ...
            item-key="id"
            handle=".cursor-grab"
            @end="onSectionsReorder"
          >
            <template #item="{ element: section }">
              <div
                class="flex items-center gap-2 p-2 lg:p-3 rounded-lg bg-bark-900/50 border border-bark-800"
              >
                ...
                <span class="flex-1 font-medium text-white truncate text-sm lg:text-base">{{
                  section.name
                }}</span>
                <div class="flex items-center gap-0.5 lg:gap-1">
                  <button
                    @click="editSection(section)"
                    class="p-1.5 lg:p-2 rounded-lg text-bark-400 hover:text-white hover:bg-bark-800 touch-manipulation"
                  >
                    <svg
                  ...
                  <button
                    ...
                    class="p-1.5 lg:p-2 rounded-lg text-bark-400 hover:text-red-400 hover:bg-red-900/20 touch-manipulation"
                  >
                    <svg
                      class="w-3.5 h-3.5 lg:w-4 lg:h-4"
                      fill="none"
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="flex gap-2 mb-4">
            ...
            />
            <select
              v-model="newMappingSection"
              class="bg-bark-800 border border-bark-700 text-white text-sm rounded-lg px-2 lg:px-3 py-2 focus:outline-none focus:ring-2 focus:ring-forest-500 w-32 lg:w-40"
            >
            ...
            <button
              @click="createMapping"
              :disabled="!newMappingItem.trim() || !newMappingSection"
              class="btn-primary touch-manipulation"
            >
              Add
            </button>
          </div>
          ...
          <div
            ...
            class="space-y-2 max-h-[calc(100vh-460px)] overflow-y-auto scrollbar-thin"
          >
            <div
              v-for="mapping in filteredMappings"
              :key="mapping.item_name"
              class="flex items-center justify-between p-2 lg:p-3 rounded-lg bg-bark-900/50 border border-bark-800"
            >
              <div class="flex-1 min-w-0 mr-2">
                <p class="font-medium text-white truncate text-sm lg:text-base">
                  {{ mapping.item_name }}
                </p>
                <span
                  :class="[
                  ...
                  >{{ mapping.section_name }}</span
                >
              </div>
              <select
                :value="mapping.section_id"
                @change="updateMapping(mapping.item_name, Number($event.target.value))"
                class="bg-bark-800 border border-bark-700 text-white text-xs lg:text-sm rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-forest-500 w-28 lg:w-32 flex-shrink-0"
              >
                <option
                  v-for="section in sectionsStore.sections"
                  :key="section.id"
              ...
              <button
                @click="deleteMapping(mapping.item_name)"
                class="p-1.5 lg:p-2 rounded-lg text-bark-400 hover:text-red-400 hover:bg-red-900/20 ml-2 touch-manipulation"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    stroke-linecap="round"
...
const mappings = ref<
  Array<{ item_name: string; section_id: number; section_name: string; is_manual_override: number }>
>([]);
...
const deleteConfirmTitle = computed(() =>
  itemToDelete.value?.type === 'section' ? 'Delete Section' : 'Delete Mapping'
);
...
async function createMapping() {
  ...
  try {
    await $fetch('/api/mappings', {
      method: 'POST',
      body: {
        item_name: newMappingItem.value.trim(),
        section_id: newMappingSection.value,
...
async function handleDeleteConfirm() {
  ...
  try {
    if (itemToDelete.value.type === 'section') {
      ...
      await $fetch(`/api/mappings/${encodeURIComponent(itemToDelete.value.data.item_name)}`, {
        method: 'DELETE'
      });
      await fetchMappings();
